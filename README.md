[index.html](https://github.com/user-attachments/files/26681034/index.html)
<!DOCTYPE html>
...
async function saveEditReceipt(id){
  ...
  const qty = parseFloat(document.getElementById('er-qty').value)||0;
  const price = parseFloat(document.getElementById('er-price').value)||0;
  const newCtype = document.getElementById('er-ctype').value;
  const custId = newCtype==='account' ? (parseInt(document.getElementById('er-customer').value)||null) : null;
  ...
  const newTip = parseFloat(document.getElementById('er-tip').value)||0;
  const custName = newCtype==='account'
    ? (document.getElementById('er-customer').options[document.getElementById('er-customer').selectedIndex]?.text||oldRec.customerName)
    : (oldRec.customerName||'عميل نقدي');

  // 1. Reverse old equipment meters
  ...
  // 3. Update receipt record (use JS field names — dbUpdate calls map.toDB internally)
  await dbUpdate('receipts', id, {
    date: newDate,
    customerType: newCtype,
    customerId: custId,
    customerName: custName,
    waterBon: document.getElementById('er-wbon').value,
    loadBon: document.getElementById('er-lbon').value,
    tractorNum: document.getElementById('er-tractor').value,
    trailerNum: document.getElementById('er-trailer').value,
    materialId: matId, materialName: matName,
    materialPrice: price, qty, customerTotal,
    tip: newTip,
    payMethod: payMethod,
    equipId: equipId
  });

  // 4. Handle treasury changes
  // Remove old treasury entries related to this receipt
  const allTr = await dbGetAll('treasury');
  const oldTrEntries = allTr.filter(t => t.ref === id);
  for(const t of oldTrEntries) await dbDelete('treasury', t.id);

  // Add new treasury entries based on new values
  if(payMethod === 'cash'){
    await dbPut('treasury', { date: newDate, type:'in', category:'مبيعات كاش', description:`إيصال تحميل - ${custName} - ${matName}`, amount: customerTotal, ref: id });
  }
  if(newTip > 0){
    await dbPut('treasury', { date: newDate, type:'in', category:'اكرامية مكتب', description:`اكرامية - ${custName}`, amount: newTip, ref: id });
  }

  // 5. Recalculate customer balances (old and new customer if changed)
